import type { AlarmData, LevelConfig } from '../types/game';

// 24 Alarms Pool (Turkish & English)
export const ALARM_DB: AlarmData[] = [
  ...
  },
  {
    id: 'p1_ransomware',
    priority: 'P1',
    title: { en: 'Ransomware on NAS', tr: 'NAS üzerinde Fidye Yazılımı' },
    description: { en: 'Rapid file encryption activity detected on corporate NAS storage.', tr: 'Kurumsal NAS depolama biriminde hızlı dosya şifreleme aktivitesi tespit edildi.' },
    correctOrder: ['step_find_ip', 'step_isolate_client', 'step_block_nas', 'step_restore_nas'],
    steps: [
      { id: 'step_isolate_client', text: { en: 'Disconnect infected client from network', tr: 'Enfekte istemci bilgisayarı ağdan çıkar' } },
      { id: 'step_block_nas', text: { en: 'Block write access port on NAS console', tr: 'NAS konsolunda yazma erişim portunu engelle' } },
      { id: 'step_find_ip', text: { en: 'Find source IP from NAS audit logs', tr: 'NAS denetim loglarından kaynak IP adresini bul' } },
      { id: 'step_restore_nas', text: { en: 'Restore NAS from immutable snapshots', tr: 'NAS verilerini değişmez (immutable) anlık görüntülerden geri yükle' } },
    ]
  },
  ...
  },
  {
    id: 'p2_api_gateway',
    priority: 'P2',
    title: { en: 'API Gateway Latency Spike', tr: 'API Gateway Gecikme Artışı' },
    description: { en: 'Public API gateway latency exceeded 2000ms.', tr: 'Genel API ağ geçidi gecikmesi 2000ms sınırını aştı.' },
    correctOrder: ['step_curl', 'step_analytics', 'step_restart_api'],
    steps: [
      { id: 'step_curl', text: { en: 'Run curl test to check API response latency', tr: 'API yanıt gecikmesini ölçmek için curl testi çalıştır' } },
      { id: 'step_analytics', text: { en: 'Check API analytics for rate limit violations', tr: 'Hız sınırı (rate limit) ihlalleri için API analitiklerini kontrol et' } },
      { id: 'step_restart_api', text: { en: 'Restart API Gateway service container', tr: 'API Gateway servis konteynerini yeniden başlat' } }
    ]
  },
  ...
  },
  {
    id: 'p3_ad_lockout',
    priority: 'P3',
    title: { en: 'AD Account Lockout Storm', tr: 'AD Hesap Kilitleme Fırtınası' },
    description: { en: 'Multiple user accounts are locking out simultaneously due to failed authentication.', tr: 'Aynı anda birden fazla kullanıcı hesabı hatalı şifre denemeleriyle kilitleniyor.' },
    correctOrder: ['step_ad_logs', 'step_source_ip', 'step_block_ip'],
    steps: [
      { id: 'step_ad_logs', text: { en: 'Check AD domain controller security logs', tr: 'AD domain controller güvenlik loglarını kontrol et' } },
      { id: 'step_source_ip', text: { en: 'Identify source IP of lockout requests', tr: 'Kilitleme isteklerinin kaynak IP adresini tespit et' } },
      { id: 'step_block_ip', text: { en: 'Block source IP at the internal firewall', tr: 'Kaynak IP adresini iç güvenlik duvarında engelle' } }
    ]
  },
  ...
  },
  {
    ...
    correctOrder: ['step_cable', 'step_reboot_av', 'step_helpdesk'],
    steps: [
  ...
  },
  {
    ...
    correctOrder: ['step_ping_reader', 'step_poe', 'step_facilities'],
    steps: [
  ...
  },
  {
    id: 'p4_ssl_warning',
    priority: 'P4',
    title: { en: 'SSL Expiry (7 Days)', tr: 'SSL Bitiş Uyarısı (7 Gün)' },
    description: { en: 'Staging environment certificate will expire in 7 days.', tr: 'Staging (test) ortamı SSL sertifikasının süresi 7 gün içinde doluyor.' },
    correctOrder: ['step_csr', 'step_request_cert', 'step_install_cert'],
    steps: [
      { id: 'step_csr', text: { en: 'Generate Certificate Signing Request (CSR)', tr: 'Sertifika İmzalama Talebi (CSR) oluştur' } },
      { id: 'step_request_cert', text: { en: 'Request certificate renewal from CA', tr: 'CA üzerinden sertifika yenileme talep et' } },
      { id: 'step_install_cert', text: { en: 'Install certificate on staging web server', tr: 'Sertifikayı test ortamı web sunucusuna kur' } }
    ]
  }
];

export const LEVELS: LevelConfig[] = [
  { id: 1, pool: { P4: 2 } }, // Seviye 1: 2 Düşük Öncelikli Alarm
  { id: 2, pool: { P3: 2, P4: 1 } }, // Seviye 2: 2 Orta, 1 Düşük
  { id: 3, pool: { P2: 2, P3: 1, P4: 1 } }, // Seviye 3: 2 Yüksek, 1 Orta, 1 Düşük
  { id: 4, pool: { P1: 1, P2: 2, P3: 1, P4: 1 } }, // Seviye 4: 1 Kritik, 2 Yüksek, 1 Orta, 1 Düşük
  { id: 5, pool: { P1: 2, P2: 2, P3: 2, P4: 1 } }, // Seviye 5: 2 Kritik, 2 Yüksek, 2 Orta, 1 Düşük (Büyük Kaos!)
];
